...
train_dataloader = DataLoader(train_set, batch_size=8, collate_fn=collate, shuffle=False) 
...
for idx, (train_idx, val_idx) in enumerate(skf.split(X, y)):
    ...
    prompt_tokenizer = AutoTokenizer.from_pretrained("Luyu/co-condenser-marco") 
    answer_tokenizer = AutoTokenizer.from_pretrained("Luyu/co-condenser-marco") 
    
    epochs = 10 
    params = list(prompt_encoder.parameters()) + list(answer_encoder.parameters()) 
    ...
    prompt_encoder.zero_grad() 
    answer_encoder.zero_grad() 
    ...
    for epoch_i in tqdm(range(0, epochs), desc="Epochs", position=0, leave=True, total=epochs):
        ...
        with tqdm(train_dataloader, unit="batch") as tepoch:
            for step, batch in enumerate(tepoch): 
                ...
                prompt_embeddings = prompt_encoder(prompt_input_ids, prompt_attn_masks) 
                A_embeddings = answer_encoder(A_input_ids, A_attn_masks) 
                B_embeddings = answer_encoder(B_input_ids, B_attn_masks) 
                C_embeddings = answer_encoder(C_input_ids, C_attn_masks) 
                D_embeddings = answer_encoder(D_input_ids, D_attn_masks) 
                E_embeddings = answer_encoder(E_input_ids, E_attn_masks) 
        ...
        if best_map_3 < map_3: 
            best_map_3 = map_3 
            torch.save(prompt_encoder.state_dict(), f"cocondenser_prompt_encoder_fold_{idx+1}.pt") 
            torch.save(answer_encoder.state_dict(), f"cocondenser_answer_encoder_fold_{idx+1}.pt") 
    
    foldwise_map_scores.append(best_map_3)     
    
    
print(foldwise_map_scores) 


print(f"mean MAP@3 scores: {np.mean(foldwise_map_scores)}") 
